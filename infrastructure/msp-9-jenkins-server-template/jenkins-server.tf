...
provider "aws" {
  region = var.region
    access_key = "AKIARCKGPU7BUYUMO4NF"
    secret_key = "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"
...
resource "aws_instance" "tf-jenkins-server" {
  ...
  key_name      = var.mykey
  vpc_security_group_ids = [aws_security_group.tf-jenkins-sec-gr-muco.id]
  iam_instance_profile = aws_iam_instance_profile.tf-jenkins-server-profile-muco.name
  root_block_device {
    volume_size = 16
  }
  tags = {
...
resource "aws_security_group" "tf-jenkins-sec-gr-muco" {
...
resource "aws_iam_role" "tf-jenkins-server-role-muco" {
...
resource "aws_iam_instance_profile" "tf-jenkins-server-profile-muco" {
  name = var.jenkins-profile
  role = aws_iam_role.tf-jenkins-server-role-muco.name
}
